...
function draw() {
  ...
  //fall or not fall
  for(var i=0;i<7;i++){
    if(peopley+80+stone_speed*1.5>=stony[i]&&peopley+80-stone_speed*1.5<=stony[i]&&peoplex>=stonx[i]-50&&peoplex<=stonx[i]+100){
      peopley=stony[i]-78;
  ...
  text("SCORE:",1050,200);
  text(score,1050,250);  
  //text(stone_speed,1050,300);
  
  //speed up
  if(time%600==0){
    stone_speed=stone_speed+0.4;
  }
